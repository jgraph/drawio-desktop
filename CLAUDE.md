...
`npm test` runs the unit tests in `src/test/` (CLI argument parsing, CLI export symlink handling, MSI shortcut icon hook, System32 path helper; Node's built-in test runner). Everything else is manual testing, documented in `doc/RELEASE_PROCESS.md`:
