...
│   └── appimage-legacy-config.test.js # Drift guard for the two AppImage configs (npm test)
...
| `npm run release-linux` | Linux (AppImage, deb, rpm) |
| `npm run release-linux-legacy` | Linux AppImage only, fuse2 runtime, `legacy` update channel |
| `npm run release-appx` | Windows Store |
...
- The primary AppImage uses the static runtime (`toolsets.appimage` in `electron-builder-linux-mac.json`), so it needs no libfuse2 (#2527)
- AppImageLauncher 2.x cannot run a static-PIE runtime: its binfmt bypass copies only the runtime ELF into a memfd and redirects the runtime back to the real file with an `LD_PRELOAD` shim, which a static binary ignores. Upstream `AppImage/type2-runtime#121`, fixed only in AppImageLauncher 3.x (still beta)
- So `electron-builder-linux-legacy.json` builds a second, `-legacy` suffixed AppImage on the old fuse2 runtime for those users. It is AppImage-only and keeps the same `afterPack` fuses hook
- The two publish to separate update channels (`latest` vs `legacy`) so they never auto-update onto each other. `npm run sync -- legacy` writes `src/main/updateChannel.js`, which `electron.js` passes to `setFeedURL`. Re-run `npm run sync` afterwards so later packaging in the same job does not inherit the legacy channel
- `src/test/appimage-legacy-config.test.js` fails if the two configs drift apart, or if a legacy-flagged `updateChannel.js` is committed
